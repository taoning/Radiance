...
int jdate(/* Julian date (days into year) */
  ...
  static short mo_da[12] = {0,   31,  59,  90,  120, 151,
...
double stadj(/* solar time adjustment from Julian date */
             const int jd, const double s_meridian, const double s_longitude) {
...
double salt(/* solar altitude from solar declination and solar time */
            const double sd, const double st, const double s_latitude) {
...
static double norm_cir(double r, const double p) {
  while (r < 0)
    r += p;
  while (r >= p)
    r -= p;
...
double mjdate(const int year, const int month, const int day, double hour,
              const double s_meridian) {
...
double msdec(const double mjd, double *stp,
             const double s_longitude) { /* Ecliptic coordinates (radians) */
...
int compute_sundir(const Datetime *dts, const Location *lc, const int tsolar,
                   double sundir[3]) {
  double altitude, azimuth;
  double sd = dts->hour;
  double st = dts->hour;
  const double s_latitude = lc->latitude * DEG;
  const double s_longitude = lc->longitude * DEG;
  const double s_meridian = lc->meridian * DEG;

  if (dts->year) {
    double mjd = mjdate(dts->year, dts->month, dts->day, dts->hour, s_meridian);
    if (tsolar)
      sd = msdec(mjd, NULL, s_longitude);
    else
      sd = msdec(mjd, &st, s_longitude);
  } else {
    int jd = jdate(dts->month, dts->day);
    sd = sdec(jd);
    if (!tsolar)
      st = dts->hour + stadj(jd, s_meridian, s_longitude);
  }
  altitude = salt(sd, st, s_latitude);
  azimuth = sazi(sd, st, s_latitude);
