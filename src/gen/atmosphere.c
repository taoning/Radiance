...
static double linear_interp(double x, const double *xp, const double *fp,
                            int size) {
  if (x <= xp[0]) {
    return fp[0];
  } else if (x >= xp[size - 1]) {
...
static int hit_sphere(const FVECT orig, const FVECT dir, const double radius,
                      double *roots) {
  float A = dir[0] * dir[0] + dir[1] * dir[1] + dir[2] * dir[2];
  float B = 2 * (dir[0] * orig[0] + dir[1] * orig[1] + dir[2] * orig[2]);
  float C = orig[0] * orig[0] + orig[1] * orig[1] + orig[2] * orig[2] -
            radius * radius;
...
int tracesky(float *skyclr, const FVECT dir, const FVECT sundir,
  ...
  const double phase_m =
      3.0 / (8.0 * M_PI) * ((1.0 - ecc * ecc) * (1.0 + mu * mu)) /
      ((2.0 + ecc * ecc) * pow(1.0 + ecc * ecc - 2.0 * ecc * mu, 1.5));
  ...
  for (unsigned int i = 0; i < nsamp; ++i) {
    ...
    for (j = 0; j < nlsamp; ++j) {
      ...
      if (heightLight < 0)
        break;
      optical_depth_r_s +=
          exp(-heightLight / atmos->rayleigh->scaleheight) * seglen_s;
      optical_depth_ms_s +=
          exp(-heightLight / atmos->mie->scaleheight_s) * seglen_s;
      optical_depth_ma_s +=
          exp(-heightLight / atmos->mie->scaleheight_a) * seglen_s;
      tcurrent_s += seglen_s;
    }
    if (j == nlsamp) {
      double tau = 0;
      for (int i = 0; i < 20; i++) {
        tau = atmos->rayleigh->scoeffs0[i] *
              (optical_depth_r + optical_depth_r_s);
        tau += atmos->mie->scoeffs0[i] * miescaler *
               (optical_depth_ms + optical_depth_ms_s);
        tau += atmos->mie->acoeffs0[i] * miescaler *
               (optical_depth_ma + optical_depth_ma_s);
        sumr[i] += exp(-tau) * hr;
        summ[i] += exp(-tau) * hms;
      }
    }
  ...
  for (int i = 0; i < NSSAMP; i++) {
    skyclr[i] = (sumr[i] * atmos->rayleigh->scoeffs0[i] * phase_r +
                 summ[i] * atmos->mie->scoeffs0[i] * miescaler * phase_m) *
                EXTSOL[i] * WVLSPAN;
    // skyclr[i] = (sumr[i] * atmos->rayleigh->scoeffs0[i] * phase_r) *
    // EXTSOL[i] * WVLSPAN;
  }
...
int tracesun(float *sunclr, const FVECT sundir, const Atmosphere *atmos) {
