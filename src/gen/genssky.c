...
static void compute_mie_coefficients(double alpha, double beta, double scaleheight, double * results) {
...
static double smoothstep(const double edge0,
    const double edge1, double x) {
    x = (x - edge0) / (edge1 - edge0);
    x = x < 0.0 ? 0.0 : x > 1.0 ? 1.0 : x;
    return x * x * (3.0 - 2.0 * x);
}

static double clamp_cosine(double mu) {
    return fmax(-1.0, fmin(1.0, mu));
}

static double clamp_distance(double d) {
    return fmax(d, 0.0);
}

static double clamp_radius(double r) {
    return fmax(ER, fmin(r, AR));
}

static double safe_sqrt(double a) {
    return sqrt(fmax(a, 0.0));
}

static double distance_to_space(const double r,
    const double mu) {
    assert(r <= AR);
...
static double distance_to_earth(const double r,
    const double mu) {
    assert(r >= ER);
    assert(mu >= -1.0 && mu <= 1.0);
...
static void compute_transmittance_to_space(const double r,
    const double mu,
        float * result) {
    assert(r >= ER && r <= AR);
    assert(mu >= -1.0 && mu <= 1.0);
...
static void to_transmittance_uv(const double r,
    const double mu, double * u,
        double * v) {
    assert(r >= ER && r <= AR);
    assert(mu >= -1.0 && mu <= 1.0);
...
static void from_transmittance_uv(const double u,
...
static DATARRAY * get_transmittance_to_space(DATARRAY * dp,
    const double r,
        const double mu) {
    assert(r >= ER && r <= AR);
    assert(mu >= -1.0 && mu <= 1.0);
...
static void get_transmittance_to_sun(DATARRAY * tau_dp,
    const double r,
...
static double
distance_to_nearst_atmosphere_boundary(const double r,
    const double mu,
        const int ray_r_mu_intersects_ground) {
    if (ray_r_mu_intersects_ground) {
...
static void compute_single_scattering(DATARRAY * tau_dp,
    const double r,
        const double mu,
            const double mu_s,
                const double nu,
...
static DATARRAY * interpolate_scattering(DATARRAY * dp,
    const double r,
        const double mu,
            const double mu_s,
                const double nu,
...
static void to_irradiance_uv(const double r,
    const double mu_s, double * u,
        double * v) {
    assert(r >= ER && r <= AR);
    assert(mu_s >= -1.0 && mu_s <= 1.0);
...
static void from_irradiance_uv(const double u,
    const double v, double * r,
        double * mu_s) {
    assert(u >= 0.0 && u <= 1.0);
    assert(v >= 0.0 && v <= 1.0);
...
static DATARRAY * get_irradiance(DATARRAY * dp,
    const double r,
        const double mu_s) {
    double u, v;
...
compute_scattering_density(DATARRAY * tau_dp, DATARRAY * scat1r, DATARRAY * scat1m,
    DATARRAY * scat, DATARRAY * irrad_dp,
    const double r,
        const double mu,
            const double mu_s,
                const double nu,
...
static void compute_direct_irradiance(DATARRAY * tau_dp,
    const double r,
...
DATARRAY * allocate_3d_datarray(char * name,
    const int ri,
        const int rj,
            const int rk) {
    DATARRAY * dp;
...
DATARRAY * allocate_5d_datarray(char * name,
    const int ri,
        const int rj,
            const int rk,
                const int rl,
...
int precompute(const int sorder) {
    ...
    DATARRAY * tau_dp =
        allocate_3d_datarray("ssky_transmittance.dat", TRANSMITTANCE_TEXTURE_HEIGHT,
            TRANSMITTANCE_TEXTURE_WIDTH, NSSAMP);
    DATARRAY * delta_irradiance_dp =
        allocate_3d_datarray("ssky_delta_irradiance.dat", IRRADIANCE_TEXTURE_HEIGHT,
    ...
    DATARRAY * scattering_dp = allocate_5d_datarray(
        "ssky_scattering.dat", SCATTERING_TEXTURE_R_SIZE, SCATTERING_TEXTURE_MU_SIZE,
...
int compute_sundir(const int year,
    const int month,
        const int day,
            const double hour,
                const int tsolar,
...
int gen_spect_sky(DATARRAY * tau_dp, DATARRAY * scat_dp, DATARRAY * scat1m_dp, FVECT sundir) {
    const int angres = 3;
